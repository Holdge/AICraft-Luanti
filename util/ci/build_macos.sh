...
: "${xcodever:?xcodever is required}"
: "${osver:?osver is required}"
: "${arch:?arch is required}"
: "${build_type:?build_type is required}"
: "${DEPS_DIR:?DEPS_DIR is required}"
: "${USE_XCODE:?USE_XCODE is required}"

sudo xcode-select -s "/Applications/Xcode_${xcodever}.app/Contents/Developer"
sdk_path="$(xcrun --sdk macosx --show-sdk-path)"
sdkroot="$(realpath "${sdk_path}/../MacOSX${osver}.sdk")"
